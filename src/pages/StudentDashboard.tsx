           ...
           {activeTab === 'result' && (
             <div className="animate-in fade-in space-y-6">
                ...
                <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border-l-4 border-[#FFD700] flex flex-col md:flex-row gap-4 items-end print-hide">
                    <div className="w-full md:flex-1"><label className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2 block">Session</label><select value={selectedSession} onChange={(e) => setSelectedSession(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-900 outline-none"><option>2025/2026</option><option>2024/2025</option></select></div>
                    <div className="w-full md:flex-1"><label className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2 block">Term</label><select value={selectedTerm} onChange={(e) => setSelectedTerm(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-900 outline-none"><option>1st Term</option><option>2nd Term</option><option>3rd Term</option></select></div>
                    <button onClick={() => fetchResults(studentProfile.id, selectedTerm, selectedSession)} disabled={loading} className="w-full md:w-auto px-8 py-3 bg-red-600 text-white font-black rounded-xl shadow-lg hover:bg-red-700 transition-all uppercase tracking-wider">{loading ? 'Checking...' : 'Check Result'}</button>
                </div>

                {results.length > 0 && (
                  <div className="space-y-4">
                    <div className="flex justify-end print-hide">
                       <button onClick={() => window.print()} className="px-6 py-3 bg-black text-[#FFD700] font-black rounded-xl shadow-lg hover:bg-gray-900 transition-all flex items-center gap-2 uppercase tracking-wider"><Printer size={18}/> Print A4 Report</button>
                    </div>
                    ...
                    <div id="result-print-area" className="bg-white rounded-xl md:rounded-2xl shadow-xl border border-gray-200 mx-auto w-full max-w-[210mm] relative overflow-hidden md:overflow-visible">
                        <style>{`@media print { body * { visibility: hidden; } #result-print-area, #result-print-area * { visibility: visible; } #result-print-area { position: absolute; left: 0; top: 0; width: 100%; max-width: 100%; margin: 0; padding: 0; box-shadow: none; border: none; background: white; color: black !important; } @page { size: A4 portrait; margin: 10mm; } * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } .print-hide { display: none !important; } .page-break-avoid { page-break-inside: avoid; } }`}</style>
                        ...
                        <div className="relative z-10 p-4 sm:p-6 md:p-8">
                            <div className="flex flex-col sm:flex-row items-center justify-between border-b-[3px] border-black pb-4 mb-4 gap-4">
                               ...
                               <div className="text-center flex-1 px-2 sm:px-4"><h1 className="text-xl sm:text-2xl md:text-3xl font-black text-red-700 uppercase tracking-wide">Citadel of Knowledge</h1><h2 className="text-base sm:text-lg md:text-xl font-bold text-gray-800 uppercase tracking-widest">International School</h2><p className="text-[10px] sm:text-xs md:text-sm font-medium text-gray-600 mt-1">Adjacent First Bank, Saw-Mill Area, Lagos Road, Ilorin, Kwara State.</p></div>
                               <div className="w-16 h-20 sm:w-20 sm:h-24 border-2 border-gray-300 flex items-center justify-center bg-gray-50 shrink-0">{studentProfile?.passport_url ? <img src={studentProfile.passport_url} className="w-full h-full object-cover"/> : <span className="text-[8px] sm:text-[10px] text-gray-400 uppercase font-bold text-center px-1">Passport Photo</span>}</div>
                            </div>
                            ...
                            <div className="overflow-x-auto border-2 border-black mb-4">
                              <table className="w-full text-left border-collapse min-w-[500px]">
                                <thead className="bg-red-700 text-white font-bold text-[8px] sm:text-[9px] md:text-[11px]">
                                  <tr>
                                    <th className="p-1 border border-black uppercase text-center w-6">S/N</th>
                                    <th className="p-1 border border-black uppercase w-32">SUBJECT</th>
                                    
                                    {/* DYNAMIC PRIMARY HEADERS */}
                                    {!isSecondary && <th className="p-1 text-center border border-black leading-tight">CLASS<br/>QUIZ</th>}
                                    {!isSecondary && <th className="p-1 text-center border border-black leading-tight">HOME<br/>QUIZ</th>}
                                    
                                    <th className="p-1 text-center border border-black leading-tight">1ST CA</th>
                                    <th className="p-1 text-center border border-black leading-tight">2ND CA</th>
                                    <th className="p-1 text-center border border-black leading-tight">EXAM</th>
                                    <th className="p-1 text-center border border-black text-[#FFD700]">TOTAL</th>
                                    <th className="p-1 text-center border border-black">GRADE</th>
                                    <th className="p-1 text-center border border-black text-[#FFD700]">POS</th>
                                    <th className="p-1 text-center border border-black">REMARK</th>
                                  </tr>
                                </thead>
                                <tbody className="text-[9px] sm:text-[10px] md:text-xs">
                                   {results.map((res, i) => {
                                     ...
                                     return (
                                       <tr key={i} className="even:bg-gray-50 border-b border-gray-300">
                                         <td className="p-1 text-center border-r border-gray-300 font-bold text-gray-500">{i + 1}</td>
                                         <td className="p-1 font-bold text-gray-900 border-r border-gray-300 uppercase truncate max-w-[120px]">{res.subject}</td>
                                         
                                         {/* DYNAMIC PRIMARY DATA */}
                                         {!isSecondary && <td className="p-1 text-center font-medium border-r border-gray-300">{res.class_quiz || '-'}</td>}
                                         {!isSecondary && <td className="p-1 text-center font-medium border-r border-gray-300">{res.home_quiz || '-'}</td>}
                                         ...
                                         <td className="p-1 text-center font-black border-r border-black bg-yellow-50">{res.total_score}</td>
                                         ...
                                         <td className="p-1 text-center font-bold text-gray-600 uppercase tracking-wider truncate max-w-[80px]">{remark}</td>
                                       </tr>
                                     );
                                   })}
                                </tbody>
                            ...
                            <div className="flex flex-col md:flex-row gap-4 mb-4 page-break-avoid text-xs md:text-sm">
                               ...
                               <div className="flex-1 border-2 border-black"><div className="bg-gray-200 text-black font-bold p-1 text-center border-b-2 border-black uppercase text-[9px] sm:text-[10px] md:text-xs tracking-wider">Psychomotor Domain</div><div className="p-1">{PSYCHOMOTOR_KEYS.map(k => (<div key={k} className="flex justify-between text-[9px] sm:text-[11px] md:text-xs border-b border-gray-200 last:border-0 px-1 py-0.5"><span className="uppercase text-gray-700">{k}</span> <span className="font-black">{reportDetails?.psychomotor_skills?.[k] || '-'}</span></div>))}</div><div className="p-1 text-[8px] sm:text-[9px] text-center border-t border-gray-300 text-gray-500 mt-1">Scale: 5-Excellent, 4-Very Good, 3-Good, 2-Fair, 1-Poor</div></div>
                               <div className="flex-1 border-2 border-black"><div className="bg-gray-200 text-black font-bold p-1 text-center border-b-2 border-black uppercase text-[9px] sm:text-[10px] md:text-xs tracking-wider">Affective Domain</div><div className="p-1">{AFFECTIVE_KEYS.map(k => (<div key={k} className="flex justify-between text-[9px] sm:text-[11px] md:text-xs border-b border-gray-200 last:border-0 px-1 py-0.5"><span className="uppercase text-gray-700">{k}</span> <span className="font-black">{reportDetails?.affective_skills?.[k] || '-'}</span></div>))}</div></div>
                            </div>

                            <div className="space-y-4 page-break-avoid border-2 border-black p-3 sm:p-4 text-xs sm:text-sm bg-yellow-50/50">
                                <div><span className="font-bold uppercase underline text-red-800">Class Teacher's Remark:</span><span className="ml-2 font-serif italic font-medium">"{reportDetails?.class_teacher_remark || 'Satisfactory performance.'}"</span></div>
                                <div className="pt-6 mt-4 flex justify-between items-end"><div className="text-center w-1/2 pr-2"><div className="w-full max-w-[160px] mx-auto border-b border-black mb-1"></div><span className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-gray-600 line-clamp-1">Class Teacher's Signature</span></div><div className="text-center w-1/2 pl-2"><div className="w-full max-w-[160px] mx-auto border-b border-black mb-1"></div><span className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-gray-600 line-clamp-1">Principal's Signature & Date</span></div></div>
                            </div>
                        </div>
                    </div>
                  </div>
