                  ...
                  <div className="space-y-4">
                    ...
                    <div id="result-print-area" className="bg-white rounded-xl md:rounded-2xl shadow-xl border border-gray-200 mx-auto w-full max-w-[210mm] relative overflow-hidden md:overflow-visible">
                        ...
                        <div className="relative z-10 p-4 sm:p-6 md:p-8">
                            <div className="flex flex-col sm:flex-row items-center justify-between border-b-[3px] border-black pb-4 mb-4 gap-4"> <img src={logo} alt="Logo" className="w-20 h-20 sm:w-24 sm:h-24 object-contain shrink-0" /> <div className="text-center flex-1 px-2 sm:px-4"><h1 className="text-xl sm:text-2xl md:text-3xl font-black text-red-700 uppercase tracking-wide">Citadel of Knowledge</h1><h2 className="text-base sm:text-lg md:text-xl font-bold text-gray-800 uppercase tracking-widest">International School</h2><p className="text-[10px] sm:text-xs md:text-sm font-medium text-gray-600 mt-1">Adjacent First Bank, Saw-Mill Area, Lagos Road, Ilorin, Kwara State.</p></div> <div className="w-16 h-20 sm:w-20 sm:h-24 border-2 border-gray-300 flex items-center justify-center bg-gray-50 shrink-0">{studentProfile?.passport_url ? <img src={studentProfile.passport_url} className="w-full h-full object-cover"/> : <span className="text-[8px] sm:text-[10px] text-gray-400 uppercase font-bold text-center px-1">Passport Photo</span>}</div> </div>
                            <div className="text-center bg-[#FFD700] text-black border-2 border-black py-1.5 font-black uppercase text-xs sm:text-sm md:text-base shadow-sm mb-4"> TERMLY REPORT SHEET - {selectedTerm} {selectedSession} </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 text-xs md:text-sm border-2 border-black p-3 bg-gray-50 mb-4 font-medium"> <div className="sm:col-span-2 flex"><span className="w-24 sm:w-28 font-bold text-gray-700 uppercase shrink-0">Name:</span> <span className="font-black border-b border-gray-400 flex-1 truncate">{studentProfile?.full_name}</span></div> <div className="sm:col-span-2 flex"><span className="w-28 font-bold text-gray-700 uppercase shrink-0">Admission No:</span> <span className="font-black border-b border-gray-400 flex-1 truncate">{studentProfile?.admission_number}</span></div> <div className="flex"><span className="w-16 font-bold text-gray-700 uppercase shrink-0">Class:</span> <span className="font-black border-b border-gray-400 flex-1 truncate">{studentProfile?.current_class}</span></div> <div className="flex"><span className="w-16 font-bold text-gray-700 uppercase shrink-0">Gender:</span> <span className="font-black border-b border-gray-400 flex-1 truncate">{studentProfile?.gender || '-'}</span></div> <div className="sm:col-span-2 flex"><span className="w-32 font-bold text-gray-700 uppercase shrink-0">Next Term Begins:</span> <span className="font-black border-b border-gray-400 flex-1 truncate">{nextTermBegins || 'TBD'}</span></div> </div>
                            <div className="overflow-x-auto border-2 border-black mb-4">
                              <table className="w-full text-left border-collapse min-w-[500px]">
                                <thead className="bg-red-700 text-white font-bold text-[8px] sm:text-[9px] md:text-[11px]">
                                  <tr>
                                    <th className="p-1 border border-black uppercase text-center w-6">S/N</th> <th className="p-1 border border-black uppercase w-32">SUBJECT</th>
                                    ...
                                    <th className="p-1 text-center border border-black leading-tight">1ST CA</th> <th className="p-1 text-center border border-black leading-tight">2ND CA</th> <th className="p-1 text-center border border-black leading-tight">EXAM</th> <th className="p-1 text-center border border-black text-[#FFD700]">TOTAL</th> <th className="p-1 text-center border border-black">GRADE</th> <th className="p-1 text-center border border-black text-[#FFD700]">POS</th> <th className="p-1 text-center border border-black">REMARK</th>
                                  </tr>
                                </thead>
                                   ...
                                   {results.map((res, i) => {
                                     ...
                                     return (
                                       <tr key={i} className="even:bg-gray-50 border-b border-gray-300">
                                         <td className="p-1 text-center border-r border-gray-300 font-bold text-gray-500">{i + 1}</td> <td className="p-1 font-bold text-gray-900 border-r border-gray-300 uppercase truncate max-w-[120px]">{res.subject}</td>
                                         {!isSecondary && <td className="p-1 text-center font-medium border-r border-gray-300">{res.class_quiz || '-'}</td>} {!isSecondary && <td className="p-1 text-center font-medium border-r border-gray-300">{res.home_quiz || '-'}</td>}
                                         <td className="p-1 text-center font-medium border-r border-gray-300">{res.ca1_score || '-'}</td> <td className="p-1 text-center font-medium border-r border-gray-300">{res.ca2_score || '-'}</td> <td className="p-1 text-center font-medium border-r border-gray-300">{res.exam_score || '-'}</td> <td className="p-1 text-center font-black border-r border-black bg-yellow-50">{res.total_score}</td> <td className={`p-1 text-center font-black border-r border-gray-300 ${gradeColor}`}>{grade}</td> <td className="p-1 text-center font-black border-r border-gray-300 text-green-700">{res.position || '-'}</td> <td className="p-1 text-center font-bold text-gray-600 uppercase tracking-wider truncate max-w-[80px]">{remark}</td>
                                       </tr>
                                     );
                                   })}
                                </tbody>
