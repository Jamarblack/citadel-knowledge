...
const HeadTeacherDashboard = () => {
  ...
  // DYNAMIC GRADING ENGINE (Calculates correctly on the fly!)
  ...
  const fetchBroadsheet = async () => {
    ...
    if (error) {
      ...
      // STRICT FILTER: Remove Ghost Rows (0 for all scores) so they don't corrupt the average!
  ...
  const executeBatchAction = async () => {
    ...
    try {
      ...
      if (status === 'approved') {
        // PERMANENT FIX: Recalculate and overwrite the wrong database grades upon approval
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      {/* PENDING BATCH DETAIL MODAL (WITH PRIMARY COLUMNS) */}
      {selectedBatch && (
      ...
      {/* APPROVED BATCH DETAIL MODAL (WITH PRIMARY COLUMNS) */}
      {selectedApprovedBatch && (
