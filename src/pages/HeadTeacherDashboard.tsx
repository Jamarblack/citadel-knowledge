...
import { LayoutDashboard, Users, FileCheck, LogOut, Archive, Menu, CheckCircle, XCircle, User, AlertTriangle, Camera, Settings, Calendar, Megaphone, Trash2, Plus, GraduationCap, UserPlus, TrendingUp, Filter, Search, FileText, RefreshCw, Download } from "lucide-react";
...
const CLASS_ARMS: Record<string, string[]> = { "Pre-KG": [],  "KG 1": ["Gold", "Diamond", "Silver"], "KG 2": ["Candy", "Chocolate", "Strawberry"], "KG 3": ["Rose", "Vanilla", "Sweet"], "Pry 1": ["Greatness", "Glorious", "Progress"], "Pry 2": ["Mars", "Jupiter", "Venus"], "Pry 3": ["Pluto", "Neptune", "Uranus"], "Pry 4": ["South America", "North America", "Africa", "Europe"], "Pry 5": ["Asia", "Antarctica"] };
...
const HeadTeacherDashboard = () => {
  ...
  const fetchBroadsheet = async () => {
    ...
    if (allData.length > 0) {
      ...
      validData.forEach(row => {
        ...
        if (!isExcludedFromTotal(row.subject, targetClass)) {
      ...
      const processedData = Object.values(studentsMap).map((s: any) => ({ ...s, average: s.subjectCount > 0 ? Math.round(s.total / s.subjectCount) : 0 })).sort((a: any, b: any) => b.average - a.average);
  ...
  const downloadBroadsheet = () => {
    ...
    broadsheetData.forEach((student, index) => {
      const row = [ `"${student.name}"`, ...broadsheetSubjects.map(sub => student.scores[sub] !== undefined ? student.scores[sub] : '-'), student.total, student.average, index + 1 ];
      csvContent += row.join(',') + '\n';
    });
  ...
  useEffect(() => { if (!staffForm.name) return; const parts = staffForm.name.replace(/^(Mr\.|Mrs\.|Miss\.|Mr|Mrs|Miss)\s+/i, "").trim().split(" "); if (parts.length >= 2) setStaffForm(prev => ({ ...prev, email: `${parts[0][0].toLowerCase()}${parts[parts.length - 1].toLowerCase()}@citadelschool.edu.ng` })); }, [staffForm.name]);
  ...
  const generateAdmissionNo = () => { const year = new Date().getFullYear().toString().slice(-2); const randomNum = Math.floor(1000 + Math.random() * 9000); setStudentForm(prev => ({ ...prev, admission_no: `CIS/${year}/${randomNum}` })); };

  useEffect(() => { if (activeTab === 'reg-staff' && !staffForm.password) generateStaffPin(); if (activeTab === 'reg-student' && !studentForm.password) generateStudentPin(); }, [activeTab]);

  const handleRegisterStaff = async (e: React.FormEvent) => { e.preventDefault(); setLoading(true); try { const finalClass = staffForm.role === 'Teacher' ? (staffArm ? `${staffBaseClass} ${staffArm}` : staffBaseClass) : null; await supabase.from('staff').insert([{ full_name: staffForm.name, role: staffForm.role, email: staffForm.email, password_text: staffForm.password, assigned_class: finalClass, section: 'Primary' }]); toast.success(`Staff Created: ${staffForm.name}`); setStaffForm({ name: '', role: 'Teacher', email: '', password: '', section: 'Primary' }); setStaffBaseClass(""); setStaffArm(""); generateStaffPin(); fetchStats(); fetchTeachers(); } catch (error: any) { toast.error(error.message); } finally { setLoading(false); } };
  const handleRegisterStudent = async (e: React.FormEvent) => { e.preventDefault(); setLoading(true); try { const finalClass = studentArm ? `${studentBaseClass} ${studentArm}` : studentBaseClass; if (!finalClass) return toast.error("Please select a class"); await supabase.from('students').insert([{ full_name: studentForm.name, gender: studentForm.gender, admission_number: studentForm.admission_no, current_class: finalClass, dob: studentForm.dob, parent_phone: studentForm.parent_phone, parent_phone_2: studentForm.parent_phone_2, password_text: studentForm.password }]); toast.success(`Student Registered: ${studentForm.admission_no}`); setStudentForm({ name: '', gender: '', admission_no: '', dob: '', parent_phone: '', parent_phone_2: '', password: '' }); setStudentBaseClass(""); setStudentArm(""); generateStudentPin(); fetchStats(); fetchStudents(); } catch (error: any) { toast.error(error.message); } finally { setLoading(false); } };
  const handlePromotion = async () => { const fromClass = promoFromArm ? `${promoFromBase} ${promoFromArm}` : promoFromBase; const toClass = promoToArm ? `${promoToBase} ${promoToArm}` : promoToBase; if (!fromClass || !toClass) return toast.error("Please select source and destination classes."); const studentsInClass = studentList.filter(s => s.current_class === fromClass); if (studentsInClass.length === 0) return toast.error(`No students found in ${fromClass}`); if (!confirm(`Are you sure you want to promote ${studentsInClass.length} students from ${fromClass} to ${toClass}?`)) return; setLoading(true); try { await supabase.from('students').update({ current_class: toClass }).eq('current_class', fromClass); toast.success(`Successfully promoted students to ${toClass}!`); setPromoFromBase(""); setPromoFromArm(""); setPromoToBase(""); setPromoToArm(""); fetchStudents(); } catch (error: any) { toast.error(error.message); } finally { setLoading(false); } };

  const filteredStudents = studentList.filter(s => { const matchSearch = s.full_name.toLowerCase().includes(studentSearch.toLowerCase()) || (s.admission_number || "").toLowerCase().includes(studentSearch.toLowerCase()); const matchClass = studentClassFilter === "All" ? true : s.current_class === studentClassFilter; return matchSearch && matchClass; });
  ...
  const executeBatchAction = async () => {
    ...
    try {
      ...
      setConfirmAction(null); setSelectedBatch(null); fetchStats(); fetchPendingResults(); fetchApprovedResults();
    } catch (e: any) { toast.error("Error updating results: " + e.message); } finally { setLoading(false); }
  };

  const deleteSingleResult = async (id: string) => { if (!confirm("Are you sure you want to delete this specific student's result?")) return; setLoading(true); await supabase.from('results').delete().eq('id', id); toast.success("Result deleted"); fetchApprovedResults(); fetchStats(); if (selectedApprovedBatch) { setSelectedApprovedBatch({ ...selectedApprovedBatch, results: selectedApprovedBatch.results.filter(r => r.id !== id), student_count: selectedApprovedBatch.student_count - 1 }); } setLoading(false); };
  const deleteBatchResults = async (batch: ResultBatch) => { if (!confirm(`Are you sure you want to completely delete ALL results for ${batch.class_level} ${batch.subject}? This action cannot be undone.`)) return; setLoading(true); const ids = batch.results.map(r => r.id); await supabase.from('results').delete().in('id', ids); toast.success("Batch completely deleted!"); fetchApprovedResults(); fetchStats(); setSelectedApprovedBatch(null); setLoading(false); };
          ...
          {activeTab === 'approvals' && ( 
             <div className="space-y-6 animate-in fade-in"> 
               <div className="flex justify-between items-center"> 
                  ...
                  <span className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-xs font-bold">{pendingBatches.length} Batches Pending</span> 
               </div> 

               <div className="bg-white p-4 rounded-2xl shadow-sm border border-emerald-100 flex items-center relative">
               ...
               {filteredPendingBatches.length > 0 ? ( 
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> 
                   {filteredPendingBatches.map(batch => ( 
                     <div key={batch.id} onClick={() => setSelectedBatch(batch)} className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-emerald-300 transition-all cursor-pointer group"> 
                       <div className="flex justify-between items-start mb-4"> 
                         <div className="w-12 h-12 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors"><FileCheck size={24} /></div> 
                       ...
                       <p className="text-sm font-medium text-gray-500 mb-1">{batch.class_level}</p> 
                       <div className="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded mt-2 inline-block font-bold tracking-widest uppercase">{batch.term} • {batch.session}</div> 
                       <div className="flex items-center gap-3 text-xs text-gray-400 border-t pt-4 mt-4"> 
                 ...
                 <div className="bg-white p-12 text-center rounded-2xl border border-dashed border-gray-300 text-gray-400"> 
                   <CheckCircle size={48} className="mx-auto mb-3 opacity-20 text-emerald-500"/> 
          ...
          {activeTab === 'manage-results' && ( 
             <div className="space-y-6 animate-in fade-in"> 
               <div className="flex justify-between items-center"> 
                 ...
                 <span className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-xs font-bold">{approvedBatches.length} Batches</span> 
               </div> 
               ...
               {filteredApprovedBatches.length > 0 ? ( 
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> 
                   {filteredApprovedBatches.map(batch => ( 
                     <div key={batch.id} onClick={() => setSelectedApprovedBatch(batch)} className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-emerald-300 transition-all cursor-pointer group"> 
                       <div className="flex justify-between items-start mb-4"> 
                         <div className="w-12 h-12 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors"><FileCheck size={24} /></div> 
                         <span className="text-xs font-bold bg-emerald-100 text-emerald-700 px-2 py-1 rounded">Approved</span> 
                       </div> 
                       <h3 className="text-lg font-bold text-gray-800">{batch.subject}</h3> 
                       <p className="text-sm font-medium text-gray-500 mb-1">{batch.class_level}</p> 
                       <div className="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded mt-2 inline-block font-bold tracking-widest uppercase">{batch.term} • {batch.session}</div> 
